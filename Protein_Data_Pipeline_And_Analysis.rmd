...
protein_data_raw <- read_excel(path=xlsx_path, skip=3) # skip=3 to omit the description rows

# Remove rows where the # column is a NA
protein_data_raw <- protein_data_raw[!is.na(protein_data_raw$`#`), ]
...
taxonomy_col <- which(colnames(protein_data_raw) == "Taxonomy")

# All columns after Taxonomy are numeric samples
sample_cols <- (taxonomy_col + 1):ncol(protein_data_raw)
...
test_filtered <- filter_numeric_protein_data(df_numeric)
```

```{r}
# Assuming df_numeric is your dataframe with rownames = GeneSymbols
heatmap_plot <- plot_interactive_protein_heatmap(test_filtered,
                                                 cluster_rows = FALSE,
...
```{r}
# Will prompt user
plot_protein_venn(test_filtered)
```

# for pca (not entirely sure what im doing/looking for here)
```{r}
# for pca
run_pca <- function(df, transpose = FALSE, interactive = TRUE, label_points = FALSE) {
  # --- Step 1: ensure numeric matrix ---
  if (!is.matrix(df)) {
    df <- as.data.frame(df)
  }
  
  # If first column is character (gene/protein names), use as rownames
  ...
  if (label_points) {
    p <- p + geom_text(aes(label = Label), vjust = 1.5, size = 3)
  }
  
  # --- Step 5: interactive or static ---
  if (interactive) {
...
p <- run_pca(test_filtered, transpose = FALSE, interactive = TRUE, label_points = FALSE)
p
```
